...
export type ObjectSchema<T extends Record<string, Schema>> = BaseSchema<[T] extends [never] ? Record<string, unknown> : {
...
export type ArraySchema<T extends Schema> = BaseSchema<[T] extends [never] ? unknown[] : SchemaValues<T>[], typeof SchemaType.ARRAY, ArrayRules> & {
...
export type BaseResolvedSchema<T = unknown> = {
    path: string;
    rules: ResolvedRules[];
    mutable: boolean;
    required: boolean;
...
export type ResolvedStringSchema<T extends StringSchema> = Omit<T, keyof BaseResolvedSchema> & BaseResolvedSchema & {
    private: boolean;
};
export type ResolvedDateStringSchema<T extends DateStringSchema> = Omit<T, keyof BaseResolvedSchema> & BaseResolvedSchema & {
    private: boolean;
};
export type ResolvedNumberSchema<T extends NumberSchema> = Omit<T, keyof BaseResolvedSchema> & BaseResolvedSchema & {
    private: boolean;
};
export type ResolvedOptionsSchema<T extends OptionsSchema<JsonPrimitive>> = Omit<T, keyof BaseResolvedSchema> & BaseResolvedSchema;
export type ResolvedObjectSchema<T extends ObjectSchema<never> = ObjectSchema<never>> = Omit<T, keyof BaseResolvedSchema | 'fields'> & BaseResolvedSchema & {
    fields: {
        [A in keyof T['fields']]: ResolvedSchema<T['fields'][A]>;
    };
};
export type ResolvedArraySchema<T extends ArraySchema<never>> = Omit<T, keyof BaseResolvedSchema | 'schema'> & BaseResolvedSchema & {
    defaultSchema: ResolvedSchema<T['schema']>;
    schema: ResolvedSchema<T['schema']>[];
};
export type ResolvedSchema<T extends Schema = Schema> = T extends StringSchema ? ResolvedStringSchema<T> : T extends NumberSchema ? ResolvedNumberSchema<T> : T extends ObjectSchema<never> ? ResolvedObjectSchema<T> : T extends ArraySchema<never> ? ResolvedArraySchema<T> : T extends DateStringSchema ? ResolvedDateStringSchema<T> : T extends OptionsSchema<JsonPrimitive> ? ResolvedOptionsSchema<T> : never;
/**
 * CONTEXT
 * being used in resolving and / or validation
 */
export type ValidationContext<T extends Schema = Schema> = {
    type: 'validate';
...
export type Context<T extends Schema = Schema> = ValidationContext<T> | ResolveContext<T>;
/***
 *
 *
 * VALIDATION
...
export type BaseErrorMessage<T = unknown, A = unknown, S extends ResolvedSchema = ResolvedSchema> = {
    path: string;
    schema: S;
    value: T;
    current: A;
