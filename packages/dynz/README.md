...
```typescript
import { object, string, min, max, email, number, regex, isNumeric, options } from "dynz";
...
```typescript
import { eq, and, or, conditional, email, min, object, oneOf, regex, string, options } from 'dynz'
...
```typescript
import { after, dateString, equals, max, min, object, ref, string, validate } from 'dynz'
...
const signupSchema = object({
  fields: {
    ...
    birthYear: dateString({ format: 'yyyy', rules: [min('1900'), max('2024')] }),
    graduatedAt: dateString({
      format: 'yyyy',
      rules: [after(ref('birthYear'), 'Graduation date must be after birth year')]
    })
  }
})
```
...
const orderSchema = object({
  fields: {
    ...
    shippingMethod: options({
      ...
      required: or(eq('orderType', 'express'), eq('orderType', 'international'))
    }),
    customsInfo: object({
      fields: {
