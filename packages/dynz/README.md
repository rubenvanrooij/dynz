...
- **Standard Schema compliant** — every schema implements [Standard Schema](https://standardschema.dev), so it plugs straight into tRPC, TanStack Form/Router, Hono and any other library that accepts one.
...
Every schema built with the fluent builders implements [Standard Schema](https://standardschema.dev) v1, so it can be passed to any library that accepts one — no adapter needed:

```typescript
import * as d from "dynz";

const signup = d.object({
  email: d.string().email(),
  age: d.number().min(18),
});

const result = await signup["~standard"].validate({
  email: "ada@example.com",
  age: 36,
});
// { value: { email: "ada@example.com", age: 36 } }
// or { issues: [{ message: "...", path: ["age"] }] }
```

The spec's `validate(value)` takes no extra arguments, so the built-in `~standard` validates without current values (no mutability checks) and without custom rules. Use `standardSchema()` to supply those, or to upgrade a plain-object or deserialized schema:

```typescript
const standard = d.standardSchema(schema, {
  currentValues, // enables mutability enforcement
  customRules: { passwordStrength: passwordStrengthRuleValidator },
  messageTransformer: (error) => t(error.code), // defaults to error.message
});

const fromJson = d.standardSchema(JSON.parse(serialized));
```

`~standard` is non-enumerable, so `serialize()` output is unchanged.
...
- `schema["~standard"].validate(value)` - [Standard Schema](https://standardschema.dev) validation, built into every fluent schema
- `standardSchema(schema, options?)` - Standard Schema for plain/deserialized schemas, or with `currentValues`, `customRules` and `messageTransformer`
