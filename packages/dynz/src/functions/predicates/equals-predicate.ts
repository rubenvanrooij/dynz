...
export const equalsPredicateType = "eq";

export type EqualsPredicate<TLeft extends ParamaterValue = never, TRight extends ParamaterValue = never> = {
  type: typeof equalsPredicateType;
...
export function eq<const TLeft extends ParamaterValue, const TRight extends ParamaterValue>(
  left: TLeft,
  right: TRight
): EqualsPredicate<TLeft, TRight> {
  return {
    type: equalsPredicateType,
    left,
    right,
  };
}

export function equalsPredicate(left: ValueType | undefined, right: ValueType | undefined): boolean {
  return left === right;
}
