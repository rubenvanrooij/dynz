...
export function isIncluded<T extends Schema>(
  schema: T,
  path: string,
  values: unknown,
): boolean {
  ...
  switch (condition.type) {
    case ConditionType.AND:
      return condition.conditions.every((condition) =>
        resolveCondition(condition, path, context),
      );
...
function getConditionOperands<T extends ValueType>(
  ...
  return {
    left: left,
    right: validateSchema(unpacked.schema, unpacked.value)
      ? toCompareType(unpacked.schema, unpacked.value)
      : undefined,
  };
}
...
export function resolveRules(
  ...
  return (schema.rules || [])
    ...
    .map(
      (rule): ResolvedRules =>
        rule.type === RuleType.CONDITIONAL ? rule.then : rule,
    );
}
...
export function unpackRefValue(
...
export function unpackRef(
  ...
  | { value: unknown; static: true }
  | { schema: Schema; value: unknown; static: false } {
  if (isReference(valueOrRef)) {
    const { schema, value } = getNested(
...
export function findSchemaByPath<T extends Schema = Schema>(
  ...
  const nestedSchema = path
    ...
    .reduce<Schema>((prev, cur) => {
      if (prev.type === SchemaType.ARRAY) {
        if (!isNumber(+cur)) {
          throw new Error(
            `Expected an array index at path ${path}, but got ${cur}`,
          );
        }
  ...
  if (type !== undefined && nestedSchema.type !== type) {
    throw new Error(
      `Expected schema of type ${type} at path ${path}, but got ${nestedSchema.type}`,
    );
  }
      ...
      (acc, cur) => {
        if (acc.schema.type === SchemaType.ARRAY) {
          if (!Array.isArray(acc.value)) {
            throw new Error(
              `Expected an array at path ${path}, but got ${typeof acc.value}`,
            );
          }
