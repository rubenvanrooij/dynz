...
function _validate(schema, values, path, context, parentValue) {
     ...
     */
    if (!(0, resolve_1.isIncluded)(schema, path, context)) {
     ...
     */
    if (context.validateMutable && (0, resolve_1.isMutable)(schema, path, context)) {
     ...
     * Validate required
     */
    if ((0, resolve_1.isRequired)(schema, path, context) && newValue === undefined) {
        return {
            success: false,
            errors: [
                {
...
function validateEqualsRule(schema, path, rule, value, context) {
    const equals = (0, resolve_1.unpackRef)(rule.value, path, context);
    return equals === value
        ? undefined
...
function validateMinRule(schema, path, rule, value, context) {
    const min = assertType([types_1.SchemaType.DATE_STRING, types_1.SchemaType.NUMBER], (0, resolve_1.unpackRef)(rule.min, path, context));
    const validate = () => {
        switch (schema.type) {
            case types_1.SchemaType.NUMBER:
                return assertType(schema.type, value) >= assertNumber(min);
            case types_1.SchemaType.STRING:
                return assertType(schema.type, value).length >= assertNumber(min);
...
function validateMaxRule(schema, path, rule, value, context) {
    const max = assertType([types_1.SchemaType.DATE_STRING, types_1.SchemaType.NUMBER], (0, resolve_1.unpackRef)(rule.max, path, context));
    const validate = () => {
        switch (schema.type) {
            case types_1.SchemaType.NUMBER:
                return assertType(schema.type, value) <= assertNumber(max);
            case types_1.SchemaType.STRING:
                return assertType(schema.type, value).length <= assertNumber(max);
            case types_1.SchemaType.OBJECT:
                return (Object.keys(assertType(schema.type, value)).length <=
                    assertNumber(max));
            case types_1.SchemaType.ARRAY:
...
function validateMaxPrecision(schema, path, rule, value, context) {
    const maxPrecision = assertNumber((0, resolve_1.unpackRef)(rule.max, path, context));
    return maxPrecision <= getPrecision(assertNumber(value))
        ? undefined
        : {
...
function assertDateString(value) {
    return assertType(types_1.SchemaType.DATE_STRING, value);
}
/**
