 ...
 * wrapped with `v()`/`val()`/`st()` (i.e. `{ type: STATIC_TYPE }`) are statically
...
export function resolveStatic<T extends ValueType = ValueType>(
  ...
  if (value !== undefined && typeof value === "object" && "type" in value && value.type === STATIC_TYPE) {
